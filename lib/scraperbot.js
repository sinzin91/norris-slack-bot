...
ScraperBot.prototype._onMessage = function (message) {
    if (this._isChatMessage(message) &&
        ...
        this._replyWithScraperId(message);
    }
};
...
ScraperBot.prototype._replyWithScraperId = function (originalMessage) {
    var self = this;
    self.db.get('SELECT id FROM external_crawl_settings WHERE store_id = 245131', function (err, record) {
        if (err) {
            return console.error('DATABASE ERROR:', err);
        }
