...
ScraperBot.prototype._replyWithScraperIds = function (originalMessage, store_id) {
    var self = this;
    self.db.all('SELECT crawl_store_id FROM external_crawl_settings WHERE store_id = ?', store_id, function (err, record) {
        if (err) {
            return console.error('DATABASE ERROR:', err);
        }
...
ScraperBot.prototype._replyWithScraperId = function (originalMessage, name) {
    var self = this;
    self.db.get('SELECT id FROM external_crawl_sites WHERE name = ?', name, function (err, record) {
        if (err) {
            return console.error('DATABASE ERROR:', err);
        }
...
ScraperBot.prototype._replyWithScraperName = function (originalMessage, id) {
    var self = this;
    self.db.get('SELECT name FROM external_crawl_sites WHERE id = ?', id, function (err, record) {
        if (err) {
            return console.error('DATABASE ERROR:', err);
        }
